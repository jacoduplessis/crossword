/* App.html generated by Svelte v1.54.0 */
...
`
  ...
  this.observe('columns', this.updateGrid)
	...
	function encapsulateStyles(node) {
		setAttribute(node, "svelte-2681975636", "");
	}

	function add_css() {
		var style = createElement("style");
		style.id = 'svelte-2681975636-style';
		style.textContent = "[svelte-2681975636]{box-sizing:border-box;font-family:monospace}input[svelte-2681975636]{padding:.5rem 0.8rem;border:1px solid #000}.cell[svelte-2681975636]{text-transform:uppercase;font-size:20px;font-weight:600;width:100%}.cell[svelte-2681975636]:focus{background-color:lightyellow}.black[svelte-2681975636]{background-color:#000 !important;color:#000 !important}.empty[svelte-2681975636]{color:#fff !important}.grid[svelte-2681975636]{display:grid;grid-gap:0;justify-content:center}#container[svelte-2681975636]{padding:1rem;max-width:800px;margin-left:auto;margin-right:auto}label[svelte-2681975636]{margin-left:0.6rem;margin-right:0.3rem}button[svelte-2681975636],label[svelte-2681975636]{text-transform:uppercase;font-weight:600}button[svelte-2681975636]{padding:0.5rem 0.8rem;background-color:rgba(0, 0, 0, 0);border:1px solid #000}button[svelte-2681975636]:hover{background-color:lightsteelblue;cursor:pointer}button.active[svelte-2681975636]{background-color:black;color:white}.cell-container[svelte-2681975636]{position:relative}.cell-number[svelte-2681975636]{position:absolute;font-size:0.7rem;font-weight:600;top:0.1rem;left:0.2rem}.help[svelte-2681975636]{margin-bottom:1rem;max-width:600px;margin-left:auto;margin-right:auto;padding:1rem;border:1px solid black}";
	...
	function create_main_fragment(state, component) {
		...
		return {
			...
			p: function update(changed, state) {
				...
				if (changed.numbers || changed.blacks || changed.values || changed.size) {
					for (var i = 0; i < each_value.length; i += 1) {
						if (each_blocks[i]) {
	...
	function create_if_block(state, component) {
		...
		return {
			...
			p: function update(changed, state) {
				if (!input_updating) input.value = state.rows;
				if (!input_1_updating) input_1.value = state.columns;
				input_2.checked = state.symmetric;
	...
	function create_if_block_1(state, component) {
		...
		return {
			...
			p: function update(changed, state) {
				if (!input_updating) input.value = state.shareURL;
			},
	...
	function create_if_block_2(state, component) {
		...
		return {
			c: function create() {
				div = createElement("div");
				div.innerHTML = "<p svelte-2681975636>Enter a dot make a cell black.</p>\n    <p svelte-2681975636>Navigate with arrow keys.</p>";
				this.h();
			},
	...
	function create_each_block(state, each_value, val, index, component) {
		var div, text, input, input_updating = false, input_class_value, input_id_value;
		...
		function input_input_handler() {
			...
			var state = component.get();
			input_updating = true;
			context.each_value[context.index] = input.value;
			component.set({ values: state.values, size: state.size });
			input_updating = false;
		}
		...
		return {
			...
			p: function update(changed, state, each_value, val, index) {
				...
				if (!input_updating) input.value = val;
	...
	function Crossword(options) {
		...
		if (!document.getElementById("svelte-2681975636-style")) add_css();
